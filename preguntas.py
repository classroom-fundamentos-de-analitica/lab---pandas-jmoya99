...
def pregunta_11():
    ...
    39   39    a,d,f
    """
    tbl = tbl1.groupby("_c0")['_c4'].apply(list).apply(lambda x: sorted(x)).apply(lambda x: ','.join(map(str, x))).to_frame().reset_index()
    return tbl
...
def pregunta_12():
    ...
    tbl = tbl2.groupby("_c0")['_c5'].apply(list).apply(lambda x: sorted(x)).apply(lambda x: ','.join(map(str, x))).to_frame().reset_index()
    return tbl
